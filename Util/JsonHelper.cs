    ...
    public static class JsonHelper
    {
        ...
        public static DataTable JsonToDataTable(this string strJson)
        {
            ...
            for (int i = 0; i < mc.Count; i++)
            {
                ...
                {
                    object strText = strRows[r].Substring(strRows[r].IndexOf(':') + 1).Trim().Replace("，", ",").Replace("：", ":").Replace("/", "").Replace("\"", "").Trim();
                    if (strText.ToString().Length >= 5)
                    {
                        if (strText.ToString().Substring(0, 5) == "Date(")//判断是否JSON日期格式
                        {
                            strText = Convert.ToDateTime(strText.ToString()).ToString("yyyy-MM-dd HH:mm:ss");
                        }
                    }
                    dr[r] = strText;
                }
